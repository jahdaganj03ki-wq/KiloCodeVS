...
export function activate(context: vscode.ExtensionContext) {
  ...
  // Start Puter.js proxy if enabled
  let puterProxy: ProxyHandle | null = null
  const startPuter = async () => {
    const handle = await startPuterProxy(context)
    if (handle) puterProxy = handle
  }
  void startPuter()
  context.subscriptions.push({ dispose: () => stopPuterProxy(puterProxy) })

  // Start the autonomous learning engine
  const learningEngine = new LearningEngine(context)
  context.subscriptions.push(learningEngine)

  // Subscribe to SSE events for autonomous learning
  // When tool execution results arrive, feed them to the learning engine
  const unsubLearning = connectionService.onEventFiltered(
    (event) => event.type === "session.turn.close" || event.type === "session.error",
    (event) => {
      if (event.type === "session.error") {
        const errMsg = typeof event.properties.error === "string" ? event.properties.error : "session error"
        void learningEngine.onExecutionResult({
          command: errMsg,
          exitCode: 1,
          stdout: "",
          stderr: errMsg,
